...
Крупные видео на сервер заливаются **вручную** через файловый менеджер, а деплой их пропускает — по двум правилам: (1) файлы больше `DEPLOY_MAX_SIZE_MB` (по умолчанию 50 МБ, из `.env`) пропускаются по размеру; (2) отдельные файлы < 50 МБ, но нестабильные по FTP (таймаутят и подвешивают заливку), перечислены в наборе `manualSkip` в `deploy.js` и тоже всегда пропускаются. Оба списка деплой печатает в начале, чтобы не забыть залить их руками. Если добавляешь новое тяжёлое видео, которое рвёт FTP, — дописывай его путь (относительно `build/`, через `/`) в `manualSkip`.
